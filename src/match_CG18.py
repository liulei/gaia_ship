...
def main():
    ...
    cat   =   scutil.read_CG18('../../../CG18.txt')
    catb  =   scutil.read_CG18b('../../../CG18b.txt')
    cat =   np.concatenate((cat, catb), axis = 0)
...
#    print 'Total clusters in K13: %d' % (len(cat_K))
    ...
    cat =   cat[ids]
    id0 =   id0[ids]
    print 'Total cluster in CG18 with |b| < 25 degree: %d' % (len(cat))
...
#    ids =   np.arange(len(b0))
    ...
    print '# idx_in_fof    separation    r_fof    r_CG18'
    for id in ids:
